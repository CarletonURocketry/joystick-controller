...
const int hz = 1 ;// loops per second
const float wait= 1000/hz ;

const int deg_per_sec = 100; //in degress per secon 
...
void delta() {
  ...
  if (yjoy <= midy) {
    deltaphi = (((yjoy - midy) / (float)(midy)) * loop_is_delta);
  }
}
...
void loop() {
  ...
  if ((digitalRead(bottonpin) == HIGH)  and (bottonwait == 0)) {
    loop_is_delta = false;
    bottonwait == 10;
  }
  Serial.print("change");
